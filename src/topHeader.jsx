...
export default function TopBar(params) {
  const setVisi = useSetRecoilState(visibility);
    return (
      <div className="bg-white fixed right-0 left-0 md:left-[224px] z-20 h-[60px] text-2xl px-3 pr-4 flex items-center justify-between border-b-[1px] border-[#D9d9d9] shadow-sm">
        <h3 className="font-semibold flex gap-4 items-center">
          <button className="md:collapse md:w-0 rounded-full text-lg" onClick={()=>{
            setVisi(c=>!c);
          }}>M</button>
          Payouts
